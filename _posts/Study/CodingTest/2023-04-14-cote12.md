...
def DFS(x,y):
    global cnt
    cnt+=1
    board[x][y]=0                                       # 내가 밟은 곳은 다시 확인 안할 거니 0으로 만들고고
    for i in range(4):                                  # 상하좌우 4방향을을 살핀다
        xx=x+dx[i]
        yy=y+dy[i]
        if 0<=xx<n and 0<=yy<n and board[xx][yy]==1:    # 격자판 안에서 4방향중 1인곳이 있는것을 확인하면면
            DFS(xx,yy)                                  # 그곳으로 이동동
...
if __name__=="__main__":
    ...
    res=[]
    for i in range(n):
        for j in range(n):          # 격자판 행, 열 탐색색
            if board[i][j]==1:      # 1이 발견되면 장애물물
                cnt=0
                DFS(i,j)            # 발견된 장애물을 기준으로 주변을 살펴보자자
                res.append(cnt)
    print(len(res))
    res.sort()
    for x in res:
